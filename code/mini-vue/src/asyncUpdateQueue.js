...
export function queueWatcher(watcher) {
  if (!queue.includes(watcher)) {
    ...
    if (!flushing) {
      ...
      // 正在刷新 watcher 队列，比如用户 watcher 的回调函数中更改了某个响应式数据
...
/**
 * 将刷新 watcher 队列的函数或者用户调用 Vue.nextTick 方法传递的回调函数放入 callback数组
...
function nextTick(cb) {
  ...
  if (!pending) {
    ...
    // 将 flushCallbacks 函数方法如浏览器的微任务队列
...
function fulshCallbacks() {
  ...
  // 新的 flushCallbacks 函数可以进入浏览器任务队列了
  pending = false
